...
display.text("Hold=run", 32, 0, 1)
display.text("Tap=flip", 32, 8, 1)
display.text("A", 8, 28, 1)
display.text("C", 112, 28, 1)
display.text("L: ", 24, 48, 1)
display.text("R: ", 24, 56, 1)
...
while True:
    ...
    if time.ticks_diff(time.ticks_ms(), last_update_time) > 50:
        ...
        display.text(arrows[left_dir + 1], 0, y, 1)

        display.fill_rect(120, 0, 8, 64, 0)
        y = 28 + -right_dir * right_speed // 225
        display.text(arrows[right_dir + 1], 120, y, 1)

        display.fill_rect(40, 48, 64, 16, 0)
        left_encoder, right_encoder = encoders.get_counts()
        display.text(f"{left_encoder:>8}", 40, 48, 1)
        display.text(f"{right_encoder:>8}", 40, 56, 1)
